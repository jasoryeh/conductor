...
public class URLDownloader extends Downloader {
   ...
   * @param replaceIfExists if the file is found in temporary storage or already there, do you want
   *                        to delete it and replace it?
  ...
  public boolean download() {
    ...
    if (basic) {
      String basicB64 = new String(
          Base64.encodeBase64(basicAuthString.getBytes(StandardCharsets.UTF_8)));
      huc.setRequestProperty("Authorization", "Basic " + basicB64);
      this.log("Appended authorizaton header, b64 len:" + basicB64.length());
    }
    this.credentials.credentials.forEach(
        (credentialType, stringStringMap) -> stringStringMap.forEach(huc::setRequestProperty));
    ...
    if (out.exists()) {
      this.log(
          "Deleting from temporary folder " + out.getAbsolutePath() + " | Success:" + out.delete());
    }
